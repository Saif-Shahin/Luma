...
function WifiSetup() {
    const { state, nextSetupStep } = useApp();
    ...
    return (
        <div className="w-full h-full bg-black flex items-center justify-center screen-transition p-8">
            <div className="max-w-3xl w-full">
                <h1 className="text-white text-5xl font-bold mb-4 text-center">Connect to WiFi</h1>
                <p className="text-gray-400 text-xl mb-8 text-center">
                    Select your WiFi network to continue
                </p>

                {/* Network List */}
                <div className="space-y-3 max-h-[650px] overflow-hidden mb-6">
                    ...
                    <div
                        className={`p-5 rounded-xl border-2 transition-all ${
                            wifiSelectedNetworkIndex === wifiNetworks.length
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-white text-xl font-semibold">Skip for now</p>
                                    <p className="text-gray-400 text-sm">You can connect to WiFi later in settings</p>
                                </div>
                            </div>
                            {wifiSelectedNetworkIndex === wifiNetworks.length && (
                ...
                <div className="text-center text-gray-500 text-lg">
                    <p>Use ↑↓ to navigate • Press OK to select • Press BACK to go back</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="w-full h-full bg-black flex items-center justify-center screen-transition p-8">
            <div className="max-w-4xl w-full">
                ...
                {selectedNetwork && (
                    <p className="text-gray-400 text-xl mb-8 text-center">
                        Network: <span className="text-white font-semibold">{selectedNetwork.ssid}</span>
                    </p>
                )}
                    ...
                    {wifiPassword.length > 0 && (
                        <p className="text-gray-400 text-sm mt-2">
                            ...
                            {wifiPassword.length < 8 && (
                                <span className="text-yellow-400 ml-2">(minimum 8 required)</span>
                            )}
                        </p>
                    )}
                </div>
    ...
    return (
        <div className="w-full h-full bg-black flex items-center justify-center screen-transition p-8">
            <div className="text-center max-w-2xl">
                ...
                <div className="bg-gray-800/50 rounded-2xl p-6 border-2 border-green-600">
                ...
                <p className="text-gray-400 text-lg mt-8">Continuing setup...</p>
            </div>
        </div>
    );
}
